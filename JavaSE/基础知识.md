...
> 而 Java 又将基本数据类型分为8种，分别为： `byte` 、 `short` 、 `int` 、 `long` 、 `float` 、 `double` 、 `char` 、 `boolean` ，同样该 8 种数据类型同样对应了 8 种引用数据类型
...
> 当基本类型与包装类做 `==` 运算时，包装类型会被转换为基本数据类型，从而 `==` 比较的其实为数值大小，而非地址值
...
> 对于基本类型和引用类型 `==` 的作用效果是不同的。当基本类型参与判断时比较的是值是否相同，而引用类型比较的是引用地址是否相同

?> 当基本类型与引用类型进行比较时会触发 [自动拆箱](/JavaSE/基础知识.md?id=自动拆箱与自动装箱) 之后再比较数值，这里的引用类型必须是基本类型包装类不支持其他类
