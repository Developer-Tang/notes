...
>
当线程被创建并启动以后，它既不是一启动就进入了执行状态，也不是一直处于执行状态。在线程的生命周期中，它要经过新建（New）、就绪（Runnable）、运行（Running）、阻塞（Blocked）和死亡（Dead）5种状态。尤其是当线程启动以后，它不可能一直"
霸占"着CPU独自运行，所以CPU需要在多条线程之间切换，于是线程状态也会多次在运行、阻塞之间切换
...
> - **其他阻塞(sleep/join)：**运行(running)的线程执行 Thread.sleep(long ms)或 t.join()方法，或者发出了 I/O 请求时，JVM 会把该线程置为阻塞状态。当sleep()
    状态超时、join()等待线程终止或者超时、或者 I/O处理完毕时，线程重新转入可运行(runnable)状态。
...
> run()是线程需要执行的具体逻辑，在主线线程中直接调用run()并不会在新的线程中执行方法体，而是当普通方法调用在主线程中阻塞执行方法体
...
> sleep()方法作用是让线程进行休眠，即让线程阻塞等待指定时间
...
### sleep()、wait()与join()的区别

> - sleep()阻塞该线程，并不释放锁对象
> - wait()阻塞该线程，释放锁对象给其他竞争同一个锁的线程
> - join()阻塞调用线程，等待该线程终止
