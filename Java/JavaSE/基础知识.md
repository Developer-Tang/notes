...
> 数值型 `byte` 、 `short` 、 `int` 、 `long` 、 `float` 、 `double` 以及字符型 `char` 按照占用字节数的大小比较区分为 **低精度** 与 **高精度** ,当低精度值赋值给高精度值时触发 **隐式(自动)转换** ，反之需要 **显示(强制)转换** ，`boolean` 不能参与类型转换的
...
?> 当基本类型与引用类型进行比较时会触发 **自动拆箱** 之后再比较数值，这里的引用类型必须是基本类型包装类不支持其他类
