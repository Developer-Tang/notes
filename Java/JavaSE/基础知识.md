...
数值型 byte 、 short 、 int 、 long 、 float 、 double 以及字符型 char 按照占用字节数的大小比较区分为低精度与高精度,当低精度值赋值给高精度值时触发隐式(自动)转换，反之需要 显示(强制)转换，**boolean 不能参与类型转换的**
